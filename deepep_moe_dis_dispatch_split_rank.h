...
constexpr uint32_t POLLING_THREADS_NUM = 1024U;
constexpr uint32_t PREPARE_THREADS_NUM = POLLING_THREADS_NUM;
constexpr uint32_t CALC_CNT_THREADS_NUM = 1024U;
constexpr uint32_t WQE_THREADS_NUM = 128U;
constexpr uint32_t WARP_SIZE = 32U;
constexpr uint32_t MAX_TASK_NUM_PER_LOOP = 128U;
constexpr uint32_t BUILD_THREADS_NUM = MAX_TASK_NUM_PER_LOOP;
constexpr uint32_t SEND_TASK_INFO_NUM = 4U;
// kfm FOR URMA
...
// constexpr uint32_t PORT_NUM = 2;
// constexpr uint32_t JETTY_NUM = 16;
...
__simt_vf__ __aicore__ LAUNCH_BOUND(POLLING_THREADS_NUM) inline void simt_prepare_mapping(
        ...
        __ubuf__ uint32_t* totalSendNum, 
        int32_t axisBS,
        int32_t axisK, 
        ...
        uint32_t myRankId)
{
    ...
    for (int row = tid; row < axisBS*axisK; row += POLLING_THREADS_NUM) {
    ...
    // for (uint32_t flatThreadTask = tid; flatThreadTask < expertNum * axisBS; flatThreadTask += POLLING_THREADS_NUM) {
    ...
    for (uint32_t flatThreadTask = tid; flatThreadTask < maxthreadId; flatThreadTask += POLLING_THREADS_NUM) {
        ...
        if(curTokenSlot >= offsetNum)
            continue;
        if(curTokenSlot == 0){
            atomicAdd(totalSendNum, curRankId == myRankId ? 0U : offsetNum);
        }
    ...
    // for (uint32_t curExpertId = warpId; curExpertId < expertNum; curExpertId += POLLING_THREADS_NUM / WARP_SIZE) {
...
__simt_vf__ __aicore__ LAUNCH_BOUND(BUILD_THREADS_NUM) inline void buildRemoteWqeDesc(
        __ubuf__ uint32_t* expertCntList,
        __ubuf__ int32_t* expertOrderedOffsetList,
        uint32_t expertNum,
        uint32_t beginExpertId,
        uint32_t axisBS,
        uint32_t axisK,
        uint64_t srcBaseAddr,
        uint64_t tokenCommSize,
        uint32_t expertPerRank,
        uint32_t localRankId,
        __ubuf__ uint64_t* remoteAddrList,
        uint32_t opcode,
        ...
        __ubuf__ uint32_t* finishedWqeDescNum)
{
    ...
    const uint32_t warpId = tid / WARP_SIZE;
    const uint32_t laneId = tid % WARP_SIZE;
    const uint32_t currBlock =
        (BUILD_THREADS_NUM / WARP_SIZE) * loopIdx + warpId;
    const uint32_t targetLocalExp = currBlock % expertNum;
    const uint32_t currBatchOfCurrExp = currBlock / expertNum;
    const uint32_t mySlot = currBatchOfCurrExp * WARP_SIZE + laneId;
    const uint32_t targetExpId = targetLocalExp + beginExpertId;
    const uint32_t targetRankId = targetExpId / expertPerRank;
    const uint32_t validOffsetCnt = expertCntList[targetLocalExp];

    if (targetRankId != localRankId && mySlot < validOffsetCnt) {
        const int32_t offsetValue =
            expertOrderedOffsetList[targetLocalExp * axisBS + mySlot];
        const uint32_t targetExpertOnRank = targetExpId % expertPerRank;
        const uint32_t beginRankId = beginExpertId / expertPerRank;
        const uint32_t tokenId = static_cast<uint32_t>(offsetValue) / axisK;
        const uint64_t remoteBaseAddr =
            remoteAddrList[targetRankId - beginRankId];
        const uint32_t taskOffset = atomicAdd(finishedWqeDescNum, 1U);

        localAddrList[taskOffset] = srcBaseAddr + tokenId * tokenCommSize;
        remoteWqeAddrList[taskOffset] = remoteBaseAddr +
            (targetExpertOnRank * axisBS + mySlot) * tokenCommSize;
...
__simt_vf__ __aicore__ LAUNCH_BOUND(CALC_CNT_THREADS_NUM) inline void calcExpertTokenCnt(
        __ubuf__ int32_t* expertIds,
        __ubuf__ uint32_t* expertTokenCnt,
        uint32_t expertIdsSize,
        uint32_t beginExpertId,
        uint32_t expertNum)
{
    const uint32_t tid = AscendC::Simt::GetThreadIdx<0>();
    for (uint32_t expertOffset = tid; expertOffset < expertNum;
         expertOffset += CALC_CNT_THREADS_NUM) {
        expertTokenCnt[expertOffset] = 0U;
    }
    AscendC::Simt::ThreadBarrier();

    const uint32_t endExpertId = beginExpertId + expertNum;
    for (uint32_t slot = tid; slot < expertIdsSize;
         slot += CALC_CNT_THREADS_NUM) {
        const int32_t expertId = expertIds[slot];
        if (expertId >= static_cast<int32_t>(beginExpertId) &&
            expertId < static_cast<int32_t>(endExpertId)) {
            atomicAdd(expertTokenCnt +
                (static_cast<uint32_t>(expertId) - beginExpertId), 1U);
        }
    }
    AscendC::Simt::ThreadBarrier();
}
...
private:
    ...
    __aicore__ inline void CopyToBuffer(TQue<QuePosition::VECIN, 1> inQueue, TBuf<> expertMaskBuf, TBuf<> outBuf);
    ...
    __aicore__ inline void TokenCopyToBuffer();
    ...
    __aicore__ inline void SetValidExpertInfo(
        uint32_t expInfoSize, uint32_t &validNum, bool isSelfSourceTask);
    ...
    __aicore__ inline void get_sqs_and_cqs_xb(
        GM_ADDR sqs, GM_ADDR cqs);
    __aicore__ inline void get_sqs_and_cqs_xb_local_ub(
        __ubuf__ uint8_t *sqsRaw, __ubuf__ uint8_t *cqsRaw,
        uint32_t startRankId, uint32_t endRankId);

    __aicore__ inline uint64_t GetUrmaCtrlReserveSize()
    {
        uint64_t ctrlBytes = epWorldSize_ * PORT_NUM * JETTY_NUM *
            (sizeof(HcclAiRMAWQ) + sizeof(HcclAiRMACQ));
        return Ceil(ctrlBytes, WIN_ADDR_ALIGN) * WIN_ADDR_ALIGN + URMA_CTRL_GUARD;
    }

    __aicore__ inline uint64_t GetStageDoneRegionSize()
    {
        return static_cast<uint64_t>(GetActiveStageNum()) * STAGE_DONE_STRIDE;
    }
    ...
    {
        // Each active stage core owns one cache-line-aligned completion flag.
        if (isMc2Context_) {
            return (GM_ADDR)shmemBuffer_ + 
            // WIN_STATE_OFFSET * 2 +
    ...
    const MoeDistributeDispatchV2TilingData *tilingData)
{
    ...
    const uint32_t remoteRankNum = epWorldSize_ > 0U ? epWorldSize_ - 1U : 0U;
    aivUsedURMAWrite_ = aivUsedURMAWrite_ < remoteRankNum ? aivUsedURMAWrite_ : remoteRankNum;

    aivUsedStage_ = aivUsedAllToAll_ - aivUsedURMAWrite_ - aivUsedSelfCopy_;
...
{
    const uint32_t activeStageNum = GetActiveStageNum();
    GlobalTensor<float> stageDoneGT;
    ...
    uint32_t &endTokenId, uint32_t &sendTokenNum, bool isFront)
{
    ...
    if (isFront) {
        ...
        newAivId = aivId_ - (moeUsedAivNum_ + sharedUsedAivNum_); //  aivUsedURMAWrite_
...
template <TemplateMC2TypeFullmeshClass>
__aicore__ inline void MoeDistributeDispatchV2FullMesh<TemplateMC2TypeFullmeshFunc>::CopyToBuffer(TQue<QuePosition::VECIN, 1> inQueue,
    TBuf<> expertMaskBuf, TBuf<> outBuf)
{
    // 按Token分核
    LocalTensor<float> outTensorFp32 = outBuf.Get<float>();
...
template <TemplateMC2TypeFullmeshClass>
__aicore__ inline void MoeDistributeDispatchV2FullMesh<TemplateMC2TypeFullmeshFunc>::TokenCopyToBuffer()
{
    // 使用的全局参数
    ...
    if ((aivId_ >= moeUsedAivNum_) && (sharedExpertRankNum_ != 0)) {
        ...
        CopyToBuffer(inQueue, expertMaskBuf, outBuf);
    }
}
...
{
    TBuf<> checkBuf, expertTokenCntBuf;
    ...
    tpipe_->InitBuffer(expertTokenCntBuf,
        Ceil(sendExpertNum * sizeof(uint32_t), UB_ALIGN) * UB_ALIGN);
    LocalTensor<uint32_t> expertTokenCntLT = expertTokenCntBuf.Get<uint32_t>();
    const uint32_t beginMoeExpertId =
        startExpertId > sharedExpertRankNum_ ?
        startExpertId - sharedExpertRankNum_ : 0U;
    const uint32_t endMoeExpertId =
        endExpertId > sharedExpertRankNum_ ?
        endExpertId - sharedExpertRankNum_ : 0U;
    const uint32_t moeExpertNumToCount =
        endMoeExpertId - beginMoeExpertId;
    ...
    if (sendToMoeExpTokenCnt_ > 0 && moeExpertNumToCount > 0U) {
        SyncFunc<AscendC::HardEvent::S_V>();
        AscendC::Simt::VF_CALL<calcExpertTokenCnt>(
            AscendC::Simt::Dim3{CALC_CNT_THREADS_NUM, 1, 1},
            reinterpret_cast<__ubuf__ int32_t*>(validExpertIdsTensor_.GetPhyAddr()),
            reinterpret_cast<__ubuf__ uint32_t*>(expertTokenCntLT.GetPhyAddr()),
            maskCnt, beginMoeExpertId, moeExpertNumToCount);
        SyncFunc<AscendC::HardEvent::V_S>();
    }
    ...
    for (uint32_t curExpertId = startExpertId; curExpertId < endExpertId; ++curExpertId) {
        ...
        if ((curExpertId < sharedExpertRankNum_) && (activeMaskBsCnt_ > 0)) {     // 当前处理专家id为共享专家
            ...
            curExpertCnt = static_cast<int32_t>(
                expertTokenCntLT.GetValue(curMoeExpertId - beginMoeExpertId));
        }
    ...
    uint32_t expInfoSize, uint32_t &validNum, bool isSelfSourceTask)
{
    ...
    const uint32_t localExpertNum = isShareExpertRankFlag_ ? 1U : moeExpertNumPerRank_;
    for (uint32_t index = startId_; index < endId_; index++) { // 从sendCnt中挑选当前有发送过来的卡的token数量
        uint32_t localExpertId = index;
        uint32_t srcRankId = epRankId_;
        if (!isSelfSourceTask) {
            // Remote tasks use compressed (source rank, local expert) order.
        const uint32_t sourceOrder = index / localExpertNum;
            localExpertId = index % localExpertNum;
            srcRankId = sourceOrder < epRankId_ ? sourceOrder : sourceOrder + 1U;
        }

        // recvCntWorkspace retains its (local expert, source rank) layout.
        const uint32_t statusIndex = localExpertId * epWorldSize_ + srcRankId;
        expertMapTensor_(validNum) = statusIndex;
        if (statusIndex == 0U) {
            expertLeftNumTensor_(validNum) = sendCntTensor_(statusIndex);
        } else {
            expertLeftNumTensor_(validNum) =
                sendCntTensor_(statusIndex) - sendCntTensor_(statusIndex - 1U);
        }
...
__aicore__ inline void MoeDistributeDispatchV2FullMesh<TemplateMC2TypeFullmeshFunc>::LocalWindowCopy()
{
    ...
    const uint32_t localExpertNum = isShareExpertRankFlag_ ? 1U : moeExpertNumPerRank_;
    const uint32_t remoteTaskNum =
        (epWorldSize_ > 0U ? epWorldSize_ - 1U : 0U) * localExpertNum;
    uint32_t taskNum = 0U;
    uint32_t copyCoreNum = 0U;
    uint32_t copyCoreId = 0U;
    bool isSelfSourceTask = false;

    // if (aivId_ < aivUsedStage_) {
    //     // Stage cores handle tokens received from other ranks.
    //     taskNum = remoteTaskNum;
    //     copyCoreNum = aivUsedStage_;
    //     copyCoreId = aivId_;
    // } else {
    //     // Every non-stage core, including SelfCopy cores, handles self-source data.
    //     taskNum = localExpertNum;
    //     copyCoreNum = aivNum_ - aivUsedStage_;
    //     copyCoreId = aivId_ - aivUsedStage_;
    //     isSelfSourceTask = true;
    // }

    uint32_t remoteCopyCoreNum = aivUsedStage_ + aivUsedURMAWrite_;
    if (aivId_ < remoteCopyCoreNum) {
        // Stage and URMA writer cores handle tokens received from other ranks.
        taskNum = remoteTaskNum;
        copyCoreNum = remoteCopyCoreNum;
        copyCoreId = aivId_;
    } else {
        // Self-copy and CumSum cores handle tokens produced by this rank.
        taskNum = localExpertNum;
        copyCoreNum = aivUsedSelfCopy_ + aivUsedCumSum_;
        copyCoreId = aivId_ - remoteCopyCoreNum;
        isSelfSourceTask = true;
    }

    if (copyCoreNum == 0U || copyCoreId >= copyCoreNum) {
        return;
    }

    sendNum_ = taskNum / copyCoreNum;
    const uint32_t remainderTaskNum = taskNum % copyCoreNum;
    startId_ = sendNum_ * copyCoreId;
    if (copyCoreId < remainderTaskNum) {
        startId_ += copyCoreId;
        sendNum_ += 1U;
    } else {
        startId_ += remainderTaskNum;
    }
    endId_ = startId_ + sendNum_;
    ...
    SetValidExpertInfo(expInfoSize, validNum, isSelfSourceTask);
...
template <TemplateMC2TypeFullmeshClass>
__aicore__ inline void MoeDistributeDispatchV2FullMesh<TemplateMC2TypeFullmeshFunc>::get_sqs_and_cqs_xb(
    GM_ADDR sqs, GM_ADDR cqs)
{
...
template <TemplateMC2TypeFullmeshClass>
__aicore__ inline void MoeDistributeDispatchV2FullMesh<TemplateMC2TypeFullmeshFunc>::get_sqs_and_cqs_xb_local_ub(
    __ubuf__ uint8_t *sqsRaw, __ubuf__ uint8_t *cqsRaw,
    uint32_t startRankId, uint32_t endRankId)
{
    __ubuf__ HcclAiRMAWQ *sqs =
        reinterpret_cast<__ubuf__ HcclAiRMAWQ *>(sqsRaw);
    __ubuf__ HcclAiRMACQ *cqs =
        reinterpret_cast<__ubuf__ HcclAiRMACQ *>(cqsRaw);
    __gm__ ACLSHMEMAIVUDMAInfo *udmaInfo = aclshmemi_udma_qp_info_fetch();
    for (uint32_t rankIdx = startRankId;
         rankIdx < endRankId && rankIdx < epWorldSize_; ++rankIdx) {
        if (rankIdx == epRankId_) {
            continue;
        }
        for (uint32_t portIdx = 0; portIdx < PORT_NUM; ++portIdx) {
            for (uint32_t jettyIdx = 0; jettyIdx < JETTY_NUM; ++jettyIdx) {
                const uint32_t offset = rankIdx * PORT_NUM * JETTY_NUM +
                    portIdx * JETTY_NUM + jettyIdx;
                const uint32_t localOffset =
                    (rankIdx - startRankId) * PORT_NUM * JETTY_NUM +
                    portIdx * JETTY_NUM + jettyIdx;
                __gm__ ACLSHMEMUDMAWQCtx *qpCtxEntry =
                    reinterpret_cast<__gm__ ACLSHMEMUDMAWQCtx *>(
                        udmaInfo->sqPtr + offset * sizeof(ACLSHMEMUDMAWQCtx));
                __gm__ ACLSHMEMUDMACqCtx *cqCtxEntry =
                    reinterpret_cast<__gm__ ACLSHMEMUDMACqCtx *>(
                        udmaInfo->scqPtr + offset * sizeof(ACLSHMEMUDMACqCtx));
                __gm__ ACLSHMEMUBmemInfo *remoteMemInfo =
                    reinterpret_cast<__gm__ ACLSHMEMUBmemInfo *>(
                        udmaInfo->memPtr + offset * sizeof(ACLSHMEMUBmemInfo));

                __ubuf__ HcclAiRMAWQ *sq = &sqs[localOffset];
                sq->jettyId = qpCtxEntry->wqn;
                sq->sqVA = qpCtxEntry->bufAddr;
                sq->wqeSize = static_cast<uint32_t>(1U << qpCtxEntry->baseBkShift);
                sq->sqDepth = qpCtxEntry->depth;
                sq->headAddr = qpCtxEntry->headAddr;
                sq->tailAddr = qpCtxEntry->tailAddr;
                sq->dbAddr = qpCtxEntry->dbAddr;
                sq->tp_id = remoteMemInfo->tpn;
                __gm__ uint8_t *eids = reinterpret_cast<__gm__ uint8_t *>(remoteMemInfo->eidAddr);
                for (uint32_t eidByte = 0; eidByte < 16U; ++eidByte) {
                    sq->rmtEid[eidByte] = eids[eidByte];
                }
                sq->rmtObjId = remoteMemInfo->tid;
                sq->rmtTokenValue = remoteMemInfo->rmtTokenValue;
                sq->localTokenId = 0U;

                __ubuf__ HcclAiRMACQ *cq = &cqs[localOffset];
                cq->jfcId = cqCtxEntry->cqn;
                cq->cqVA = cqCtxEntry->bufAddr;
                cq->cqeSize = static_cast<uint32_t>(1U << cqCtxEntry->baseBkShift);
                cq->cqDepth = cqCtxEntry->depth;
                cq->headAddr = cqCtxEntry->headAddr;
                cq->tailAddr = cqCtxEntry->tailAddr;
                cq->dbAddr = cqCtxEntry->dbAddr;
			}
		}
	}
}
...
{   
    ...
    dw[0] = (0u << 0)                    // sqeBbIdx = 0
                ...
                | (0ull << 48)                    // rsv1
                | (0ull << 54);                   // inlineMsgLen

    // Word 2: tpId[23:0] | sgeNum[31:24]
    // Word 3: rmtJettyOrSegId[19:0] | rsv2[31:20]
    dw[1] = (tpId << 0)
                | (1u << 24)                    // sgeNum = 1
                | (static_cast<uint64_t>(rmtObjId) << 32)
                | (0ull << 52);                   // rsv2
    ...
    // Word 6-7: rmtEidH (uint64_t)
	auto rmtEid = (JETTY_POS uint32_t *)(sq->rmtEid);
    dw[2] = rmtEid[0] | (static_cast<uint64_t>(rmtEid[1]) << 32);
    dw[3] = rmtEid[2] | (static_cast<uint64_t>(rmtEid[3]) << 32);
...
__simt_callee__ inline void simt_write_wqe_v3(JETTY_POS HcclAiRMAWQ *sq, uint32_t myHead, uint64_t localAddr, uint64_t remoteAddr, uint32_t messageLen, URMAOPCODE opcode)
{
    ...
    dw[0] = (0u << 0)                    // sqeBbIdx = 0
                ...
                | (1u << 29)          			// rmtJettyType
                | ((uint32_t)owner << 31)       // owner
                | (static_cast<uint64_t>(URMAOPCODE::OP_WRITE) << 40)
                | (0ull << 48)                    // rsv1
                | (0ull << 54);                   // inlineMsgLen
...
inline void simt_nw_mj_vf(__ubuf__ wqe_desc_array_t *wqe_desc_arr,
                uint32_t wqe_count,
                JETTY_POS HcclAiRMAWQ *sqs,
                uint32_t startRankId,
                uint32_t sendRankNum,
                uint32_t writerId,
                ...
                __ubuf__ uint64_t* dbArr,
                __ubuf__ uint32_t* headArr)
{
    const uint32_t jettyPerRank = port_num * jetty_num;
    const uint32_t ownedJettyNum = sendRankNum * jettyPerRank;
    const uint32_t tid = threadIdx.x;

    for (uint32_t rankOffset = tid; rankOffset < sendRankNum;
         rankOffset += WQE_THREADS_NUM) {
        dstRankWqeCount[rankOffset] = 0U;
    }
    for (uint32_t localSqIdx = tid; localSqIdx < ownedJettyNum;
         localSqIdx += WQE_THREADS_NUM) {
        sqWqeCount[localSqIdx] = 0U;
        sqBaseHead[localSqIdx] = 0U;
        sqCursor[localSqIdx] = 0U;
        dbArr[localSqIdx] = 0U;
        headArr[localSqIdx] = 0U;
    }
    __syncthreads();

    for (uint32_t wqeIdx = tid; wqeIdx < wqe_count; wqeIdx += WQE_THREADS_NUM) {
        const uint32_t dstRank = wqe_desc_arr->dstRank[wqeIdx];
        const uint32_t rankOffset = dstRank - startRankId;
        const uint32_t ordinal = atomicAdd(dstRankWqeCount + rankOffset, 1U);
        const uint32_t sqInRank = (writerId + ordinal) % jettyPerRank;
        const uint32_t localSqIdx = rankOffset * jettyPerRank + sqInRank;
        wqeSqIdx[wqeIdx] = localSqIdx;
        atomicAdd(sqWqeCount + localSqIdx, 1U);
    }
    __syncthreads();

    for (uint32_t localSqIdx = tid; localSqIdx < ownedJettyNum;
         localSqIdx += WQE_THREADS_NUM) {
        const uint32_t count = sqWqeCount[localSqIdx];
        if (count == 0U) {
            continue;
        }
        auto mySq = &sqs[localSqIdx];
        const uint32_t baseHead = atomicAdd((__gm__ uint32_t *)(mySq->headAddr), count);
        sqBaseHead[localSqIdx] = baseHead;
        dbArr[localSqIdx] = mySq->dbAddr;
        headArr[localSqIdx] = baseHead + count;
    }
    __syncthreads();

    for (uint32_t wqeIdx = tid; wqeIdx < wqe_count; wqeIdx += WQE_THREADS_NUM) {
        uint64_t localAddr, remoteAddr;
        uint32_t messageLen, dstRank;
        URMAOPCODE opcode;
        get_wqe_content(wqe_desc_arr, wqeIdx, localAddr, remoteAddr,
            messageLen, dstRank, opcode);

        const uint32_t localSqIdx = wqeSqIdx[wqeIdx];
        const uint32_t localOffset = atomicAdd(sqCursor + localSqIdx, 1U);
        const uint32_t absoluteHead = sqBaseHead[localSqIdx] + localOffset;
        simt_write_wqe_v4(&sqs[localSqIdx], absoluteHead, localAddr,
            remoteAddr, messageLen, opcode);
    }
...
template<int port_num = 1, int jetty_num = 1>
__aicore__ inline void simt_nw_mj(__ubuf__ wqe_desc_array_t *wqe_desc_arr,
        uint32_t wqe_count,
        JETTY_POS HcclAiRMAWQ *sqs,
        uint32_t startRankId,
        uint32_t sendRankNum,
        ...
        __ubuf__ uint32_t* headArr)
{
    AscendC::Simt::VF_CALL<simt_nw_mj_vf<port_num, jetty_num>>(
        AscendC::Simt::Dim3{WQE_THREADS_NUM, 1, 1}, wqe_desc_arr, wqe_count,
        sqs, startRankId, sendRankNum, writerId, wqeSqIdx,
        dstRankWqeCount, sqWqeCount, sqBaseHead, sqCursor, dbArr, headArr);
...
{
    const uint32_t remoteWriterId = aivId_ - aivUsedStage_;
    const uint32_t remoteRankNum = epWorldSize_ - 1U;
    const uint32_t baseRankNum = remoteRankNum / aivUsedURMAWrite_;
    const uint32_t remainderRankNum = remoteRankNum % aivUsedURMAWrite_;
    const uint32_t logicalSendRankNum = baseRankNum +
        (remoteWriterId < remainderRankNum ? 1U : 0U);
    if (logicalSendRankNum == 0U) {
        return;
    }
    
    const uint32_t logicalStartRank = remoteWriterId * baseRankNum +
        (remoteWriterId < remainderRankNum ?
            remoteWriterId : remainderRankNum);
    const uint32_t logicalEndRank = logicalStartRank + logicalSendRankNum;

    const uint32_t startRankId = logicalStartRank +
        (logicalStartRank >= epRankId_ ? 1U : 0U);
    const uint32_t endRankId = logicalEndRank +
        (logicalEndRank > epRankId_ ? 1U : 0U);
    const uint32_t sendRankNum = endRankId - startRankId;
    
    const uint32_t beginExpId = startRankId * moeExpertNumPerRank_;
    const uint32_t sendExpNum = sendRankNum * moeExpertNumPerRank_;
    const uint32_t ownedJettyNum = sendRankNum * PORT_NUM * JETTY_NUM;

    TBuf<> expertCntListBuf, expertOffsetListBuf, expertOrderedOffsetListBuf,
        sendTaskNumBuf, totalSendNumBuf, remoteAddrListBuf,
        tokenSendWqeDescBuf, wqeSqIdxBuf, dstRankWqeCountBuf, sqWqeCountBuf,
        sqBaseHeadBuf, sqCursorBuf, dbBuf, headBuf, stageDoneBuf,
        stageDoneWorkBuf, stageDoneSumBuf, sqsMetaBuf, cqsMetaBuf;
    tpipe_->InitBuffer(expertIdsBuf_,
        Ceil(expertIdsCnt_ * sizeof(int32_t), UB_ALIGN) * UB_ALIGN);
    ...
    tpipe_->InitBuffer(expertOrderedOffsetListBuf,
        Ceil(sendExpNum * axisBS_ * sizeof(int32_t), UB_ALIGN) * UB_ALIGN);
    tpipe_->InitBuffer(sendTaskNumBuf, UB_ALIGN);
    tpipe_->InitBuffer(totalSendNumBuf, UB_ALIGN);
    tpipe_->InitBuffer(remoteAddrListBuf,
        Ceil(sendRankNum * sizeof(uint64_t), UB_ALIGN) * UB_ALIGN);
    tpipe_->InitBuffer(tokenSendWqeDescBuf,
        Ceil(sizeof(wqe_desc_array_t), UB_ALIGN) * UB_ALIGN);
    tpipe_->InitBuffer(wqeSqIdxBuf,
        Ceil(MAX_WQE_CNT * sizeof(uint32_t), UB_ALIGN) * UB_ALIGN);
    tpipe_->InitBuffer(dstRankWqeCountBuf,
        Ceil(sendRankNum * sizeof(uint32_t), UB_ALIGN) * UB_ALIGN);
    tpipe_->InitBuffer(sqWqeCountBuf,
        Ceil(ownedJettyNum * sizeof(uint32_t), UB_ALIGN) * UB_ALIGN);
    tpipe_->InitBuffer(sqBaseHeadBuf,
        Ceil(ownedJettyNum * sizeof(uint32_t), UB_ALIGN) * UB_ALIGN);
    tpipe_->InitBuffer(sqCursorBuf,
        Ceil(ownedJettyNum * sizeof(uint32_t), UB_ALIGN) * UB_ALIGN);
    tpipe_->InitBuffer(dbBuf,
        Ceil(ownedJettyNum * sizeof(uint64_t), UB_ALIGN) * UB_ALIGN);
    tpipe_->InitBuffer(headBuf,
        Ceil(ownedJettyNum * sizeof(uint32_t), UB_ALIGN) * UB_ALIGN);
    tpipe_->InitBuffer(sqsMetaBuf,
        Ceil(ownedJettyNum * sizeof(HcclAiRMAWQ), UB_ALIGN) * UB_ALIGN);
    tpipe_->InitBuffer(cqsMetaBuf,
        Ceil(ownedJettyNum * sizeof(HcclAiRMACQ), UB_ALIGN) * UB_ALIGN);
    ...
    LocalTensor<uint32_t> sendTaskNumLT = sendTaskNumBuf.Get<uint32_t>();
    ...
    LocalTensor<uint64_t> dbLT = dbBuf.Get<uint64_t>();
    LocalTensor<uint32_t> headLT = headBuf.Get<uint32_t>();
    LocalTensor<uint8_t> sqsMetaLT = sqsMetaBuf.Get<uint8_t>();
    LocalTensor<uint8_t> cqsMetaLT = cqsMetaBuf.Get<uint8_t>();
    __ubuf__ uint8_t *sqsUB =
        reinterpret_cast<__ubuf__ uint8_t *>(sqsMetaLT.GetPhyAddr());
    __ubuf__ uint8_t *cqsUB =
        reinterpret_cast<__ubuf__ uint8_t *>(cqsMetaLT.GetPhyAddr());
    ...
    __ubuf__ wqe_desc_array_t* tokenSendWqeDesc =
        reinterpret_cast<__ubuf__ wqe_desc_array_t*>(tokenSendWqeDescU32.GetPhyAddr());
    ...
    for (uint32_t rankOffset = 0; rankOffset < sendRankNum; ++rankOffset) {
        ...
        remoteAddrListLT.SetValue(rankOffset,
            uint64_t(GetWindAddrByRankId(dstRank)) +
            expertPerSizeOnWin_ * (epRankId_ * moeExpertNumPerRank_));
    }
    SyncFunc<AscendC::HardEvent::S_V>();

    get_sqs_and_cqs_xb_local_ub(
    ...
    uint32_t finishSendNum = 0U;
    uint32_t loopIndex = 0U;
    constexpr uint32_t WARP_NUM = BUILD_THREADS_NUM / WARP_SIZE;
    const uint32_t chunkNumPerExpert = Ceil(axisBS_, WARP_SIZE);
    const uint32_t totalBlockNum = sendExpNum * chunkNumPerExpert;
    const uint32_t totalLoopNum = Ceil(totalBlockNum, WARP_NUM);
    ...
    while (finishSendNum < targetSendNum && loopIndex < totalLoopNum) {
        ...
        AscendC::Simt::VF_CALL<buildRemoteWqeDesc>(
            ...
            reinterpret_cast<__ubuf__ int32_t*>(expertOrderedOffsetListLT.GetPhyAddr()),
            sendExpNum, beginExpId, axisBS_, axisK_, srcBaseAddr,
            hCommuSize_, moeExpertNumPerRank_, epRankId_,
            reinterpret_cast<__ubuf__ uint64_t*>(remoteAddrListLT.GetPhyAddr()),
            static_cast<uint32_t>(URMAOPCODE::OP_WRITE),
            tokenSendWqeDesc->localAddr, tokenSendWqeDesc->remoteAddr,
            tokenSendWqeDesc->dstRank, tokenSendWqeDesc->messageLen,
            reinterpret_cast<__ubuf__ uint32_t*>(tokenSendWqeDesc->opcode),
            loopIndex,
            reinterpret_cast<__ubuf__ uint32_t*>(sendTaskNumLT.GetPhyAddr()));
        SyncFunc<AscendC::HardEvent::V_S>();

        uint32_t wqeCount = sendTaskNumLT.GetValue(0);
        if (wqeCount > MAX_WQE_CNT) {
            wqeCount = MAX_WQE_CNT;
        }
        if (wqeCount > 0U) {
            tokenSendWqeDesc->size = wqeCount;
            PipeBarrier<PIPE_ALL>();
            simt_nw_mj<PORT_NUM, JETTY_NUM>(tokenSendWqeDesc, wqeCount,
                reinterpret_cast<__ubuf__ HcclAiRMAWQ *>(sqsUB),
                startRankId, sendRankNum, remoteWriterId,
                ...
                reinterpret_cast<__ubuf__ uint64_t*>(dbLT.GetPhyAddr()),
                reinterpret_cast<__ubuf__ uint32_t*>(headLT.GetPhyAddr()));

            if (!stageDone) {
                WaitAllStageDone(stageDoneLT, stageDoneWorkLT, stageDoneSumLT);
                stageDone = true;
            }
            for (uint32_t localSqIdx = 0; localSqIdx < ownedJettyNum; ++localSqIdx) {
                if (sqWqeCountLT.GetValue(localSqIdx) > 0U) {
                    st_dev(headLT.GetValue(localSqIdx),
                        reinterpret_cast<__gm__ uint32_t*>(
                            dbLT.GetValue(localSqIdx)), 0);
                }
            }
            finishSendNum += wqeCount;
        }
        ++loopIndex;
    }
    DebugClock(5);
...
__aicore__ inline void MoeDistributeDispatchV2FullMesh<TemplateMC2TypeFullmeshFunc>::SelfCopyToken()
{
    ...
    const uint32_t selfCopyAivId =
        aivId_ - aivUsedStage_ - aivUsedURMAWrite_;
    ...
    const uint32_t sendExpNum = baseExpertNum +
        (selfCopyAivId < remainderExpertNum ? 1U : 0U);
    if (sendExpNum == 0U) {
        return;
    }
    ...
    const uint32_t beginExpId = epRankId_ * moeExpertNumPerRank_ + localExpertOffset;
    ...
    WaitAllStageDone(stageDoneLT, stageDoneWorkLT, stageDoneSumLT);
    const uint64_t srcBaseAddr = uint64_t(GetSendBufferAddrByTokenId(0));
    const uint64_t selfWindowBase = uint64_t(GetWindAddrByRankId(epRankId_)) +
        expertPerSizeOnWin_ * (epRankId_ * moeExpertNumPerRank_);
    GlobalTensor<XOutType> tokenSrcGT;
    GlobalTensor<XOutType> tokenDstGT;

    for (uint32_t localExpIdx = 0; localExpIdx < sendExpNum; ++localExpIdx) {
        const uint32_t targetExpId = beginExpId + localExpIdx;
        const uint32_t targetExpertOnRank =
            targetExpId % moeExpertNumPerRank_;
        const uint32_t validOffsetCnt =
            expertCntListLT.GetValue(localExpIdx);
        for (uint32_t targetExpOffset = 0;
             targetExpOffset < validOffsetCnt; ++targetExpOffset) {
            const int32_t offsetValue = expertOrderedOffsetListLT.GetValue(
                localExpIdx * axisBS_ + targetExpOffset);
            const uint32_t tokenId =
                static_cast<uint32_t>(offsetValue) / axisK_;
            const uint64_t srcAddr =
                srcBaseAddr + tokenId * hCommuSize_;
            const uint64_t dstAddr = selfWindowBase +
                (targetExpertOnRank * axisBS_ + targetExpOffset) *
                hCommuSize_;
            tokenSrcGT.SetGlobalBuffer(reinterpret_cast<__gm__ XOutType*>(srcAddr));
            tokenDstGT.SetGlobalBuffer(reinterpret_cast<__gm__ XOutType*>(dstAddr));
            DataCopy(sendTokenTempLT, tokenSrcGT,
                hCommuSize_ / sizeof(XOutType));
            SyncFunc<AscendC::HardEvent::MTE2_MTE3>();
            DataCopy(tokenDstGT, sendTokenTempLT,
                hCommuSize_ / sizeof(XOutType));
            SyncFunc<AscendC::HardEvent::MTE3_MTE2>();
        }
    }
...
__aicore__ inline void MoeDistributeDispatchV2FullMesh<TemplateMC2TypeFullmeshFunc>::Process()
{
    ...
    if ASCEND_IS_AIV {          // 全aiv处理
        ...
        if (aivId_ < aivUsedStage_) {
            ...
            TokenCopyToBuffer(); // 前面核写本地 send buffer / shared expert
            PublishStageDone();
